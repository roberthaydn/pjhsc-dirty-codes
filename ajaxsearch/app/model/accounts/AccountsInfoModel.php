...
namespace app\model\accounts 
{
	...
	class AccountsInfoModel {
		...
		public static function Create() : AccountsInfoModel {
		...
		*/
		...
		public function readData($data) {
			...
			return (function() use (&$data) {
				$id = $this->_session_id = Session::GetSession('session_accounts');
				$sql = "SELECT {$data} FROM `tbl_accounts` WHERE `id`=:id";
				...
				return (function() use (&$data, &$row) : string {
					...
					if($data==='username') 
						return $this->_username = ucwords(strtolower($this->_model->setUsername($row[$data])->getUsername()));
